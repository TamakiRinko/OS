...
void syscallSemWait(struct StackFrame *sf) {
	disableInterrupt();//??
	int index = (int32_t)sf->edx;//sem下标号
	if(sem[index].state == 0){
		pcb[current].regs.eax = -1;
		enableInterrupt();
		return;
	}
	...
	if(sem[index].value < 0){
		...
		pcb[current].blocked.prev = sem[index].pcb.next->prev;
		sem[index].pcb.next->prev = &(pcb[current].blocked);
		sem[index].pcb.next = &(pcb[current].blocked);
		pcb[current].state = STATE_BLOCKED;
		pcb[current].sleepTime = -1;//不会被时钟唤醒
		enableInterrupt();
		asm volatile("int $0x20");
...
void syscallSemPost(struct StackFrame *sf) {
	...
	if(sem[index].value <= 0){
		...
		for(i = 1; i < MAX_PCB_NUM; ++i){
			if(&(pcb[i].blocked) == sem[index].pcb.next){
				pcb[i].state = STATE_RUNNABLE;
				pcb[i].sleepTime = 0;
			}
		}
		sem[index].pcb.next = sem[index].pcb.next->next;
		sem[index].pcb.next->prev = &(sem[index].pcb);
		enableInterrupt();
		asm volatile("int $0x20");
		pcb[current].regs.eax = 0;
		return;
